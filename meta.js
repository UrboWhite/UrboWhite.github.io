(function() {
  ...
  if (kForma) {
    kForma.addEventListener('submit', function(e) {
      ...
      if (!emailOK) {
        status.className = 'k-err';
        status.textContent = '⚠ Email address is incorrect.';
        return;
      }
      ...
      .then(function(data) {
        ...
        if (data.success) {
          ...
          status.textContent = '✓ Message successfully sent! We will reply soon.';
          kForma.reset();
        } else {
          status.className = 'k-err';
      ...
      .catch(function() {
        ...
        spinner.hidden = true;
        status.className = 'k-err';
        status.textContent = '✗ No connection to server. Please check the internet and try again.';
      });
    });
  }
